...
PROJECT_NAME          ?= pacman
PROJECT_VERSION       ?= 1.0
PROJECT_BUILD_PATH    ?= _build_linux_x64
...
RAYGUI_INCLUDE_PATH   ?= ../raygui/src
...
CFLAGS = -std=gnu99 -Wall -Wno-missing-braces -Wunused-result -D_DEFAULT_SOURCE -Werror
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ifeq ($(PLATFORM_OS),BSD)
        ...
        INCLUDE_PATHS += -I$(RAYGUI_INCLUDE_PATH)
    endif
    ifeq ($(PLATFORM_OS),LINUX)
        INCLUDE_PATHS += -I$(RAYLIB_INCLUDE_PATH)
        INCLUDE_PATHS += -I$(RAYGUI_INCLUDE_PATH)
...
ifeq ($(PLATFORM),PLATFORM_ANDROID)
    ...
    MAKEFILE_PARAMS = $(PROJECT_BUILD_PATH)/$(PROJECT_NAME)
endif
ifeq ($(PLATFORM),PLATFORM_WEB)
    MAKEFILE_PARAMS += $(PROJECT_BUILD_PATH)/$(PROJECT_NAME).zip
endif

# Default target entry
# NOTE: We call this Makefile target or Makefile.Android target
all:
	$(MAKE) $(PROJECT_BUILD_PATH) $(MAKEFILE_PARAMS)
...
$(PROJECT_BUILD_PATH)/index.html:  $(addprefix $(PROJECT_BUILD_PATH)/, $(PROJECT_NAME).html)
	cp $< $@

$(PROJECT_BUILD_PATH)/$(PROJECT_NAME).zip:  $(addprefix $(PROJECT_BUILD_PATH)/, $(PROJECT_NAME).data $(PROJECT_NAME).js $(PROJECT_NAME).wasm index.html)
	echo "zipping $^"
	zip $@ $^
...
clean_shell_cmd:
	del *.o *.exe $(PROJECT_NAME).data $(PROJECT_NAME).html $(PROJECT_NAME).js $(PROJECT_NAME).wasm index.html $(PROJECT_NAME).zip /s
